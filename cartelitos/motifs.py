...
_RARE_KINDS_RE = re.compile(r"property\s+var\s+crtRareKinds\s*:\s*\[(.*?)\]", re.S)
...
def read_rare_kinds(path=None):
    """Los eventos raros que conoce el overlay, leídos de `crtRareKinds` en
    shell.qml (mismo patrón que `read_kinds`, propia lista y regex porque
    `motifKinds` y `crtRareKinds` son pools distintos)."""
    try:
        with open(path or qml_path(), encoding="utf-8") as fh:
            src = fh.read()
    except OSError:
        return []
    m = _RARE_KINDS_RE.search(src)
    if not m:
        return []
    return re.findall(r'"([^"]+)"', m.group(1))
...
def usage_rare(kinds=None):
    kinds = read_rare_kinds() if kinds is None else kinds
    return ("usage: fatal crt rare <kind> [--screen <name|idx|all>]\n"
            "kinds: " + (", ".join(kinds) if kinds else "(no se pudo leer shell.qml)")
            + "\nsin --screen elige el overlay (bsod: la pantalla con foco;"
              " nosignal/testcard: una pantalla viva sin foco); no hay 'off',"
              " un raro termina solo")


def parse_rare(argv, kinds=None, screens=None):
    """Los argumentos de `fatal crt rare` -> (evento, error).

    Mismo patrón que `parse_force`, pero sin la palabra "off": un raro no se
    apaga a mano, expira solo (`crtRareTimer`, del lado del overlay)."""
    kinds = read_rare_kinds() if kinds is None else kinds
    argv = list(argv)
    if not argv or argv[0] in ("-h", "--help"):
        return None, usage_rare(kinds)
    kind = argv[0]
    rest = argv[1:]
    screen = None
    while rest:
        ...
        if arg.startswith("--screen="):
            ...
            return None, f"unknown argument {arg!r}\n" + usage_rare(kinds)
    if kinds and kind not in kinds:
        return None, (f"unknown rare kind {kind!r}\nkinds: " + ", ".join(kinds))
    if screen is None or screen == "all":
        return {"cmd": "rare", "kind": kind, "screen": "all"}, None
    if re.fullmatch(r"-?\d+", screen):
        idx = int(screen)
        if idx < 0:
            return None, f"screen index {idx} is out of range"
        if screens is not None and idx >= len(screens):
            return None, (f"screen index {idx} is out of range "
                          f"(hay {len(screens)}: " + ", ".join(screens) + ")")
        return {"cmd": "rare", "kind": kind, "screen": idx}, None
    if screens is not None and screen not in screens:
        return None, (f"unknown screen {screen!r} — hay: " + ", ".join(screens))
    return {"cmd": "rare", "kind": kind, "screen": screen}, None


def rare_cli(argv, screens=None, kinds=None):
    """`fatal crt rare ...`. Devuelve el código de salida (mismo patrón que
    `force_cli`). `kinds` es para el test: `crtRareKinds` recién existe en
    shell.qml desde la corrida 7, paso 3 — sin pasarlo, se lee el archivo
    real como en producción."""
    ev, err = parse_rare(argv, kinds=kinds, screens=screens if screens is not None else screen_names())
    if ev is None:
        print(err)
    ...
    try:
        ...
        return 1
    print(f"rare {ev['kind']} forced on {ev['screen']}")
    return 0
